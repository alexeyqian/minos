 ...
 * */
PRIVATE int do_fork(KMESSAGE* msg){
    // find a free slot in proc table
    struct proc* p = proc_table;
    int i;
    for(i = NR_TASKS + NR_PROCS; i < PROCTABLE_SIZE; i++, p++){
        if(p->p_flags == FREE_SLOT) break;
    }

    int child_pid = i;
    kassert(p == &proc_table[child_pid]);    
    if(i >= PROCTABLE_SIZE) return -1;
...
PUBLIC void do_wait(int caller){
    ...
    for(i = 0; i < PROCTABLE_SIZE; i++, p_proc++){
...
PRIVATE int do_exec(KMESSAGE* pmsg){
    ...
    phys_copy((void*)va2la(TASK_SYS, pathname), 
    ...
    for(i = 0; i < elf_hdr->e_phnum; i++){
        ...
        if(prog_hdr->p_type == PT_LOAD){
            ...
            phys_copy((void*)va2la(src,     (void*)prog_hdr->p_vaddr),
                      (void*)va2la(TASK_SYS, mmbuf + prog_hdr->p_offset),
                      prog_hdr->p_filesz);
        }
    }

    // setup the arg stack
    kassert(pmsg->BUF_LEN >= 0);
    uint32_t orig_stack_len = (uint32_t)pmsg->BUF_LEN;
    char stackcopy[PROC_ORIGIN_STACK];
    phys_copy((void*)va2la(TASK_SYS, stackcopy),
    ...
    while(TRUE){
        ...
        switch(msgtype){
            ...
            case WAIT:
                ...
                break;
            default:
                //dump_msg("mm: unknow message", &msg);
                kassert(0);
                break;
        }
